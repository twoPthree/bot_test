...
async def voice_message_handle(update: Update, context: CallbackContext):
    ...
    #TODO тут можно сделать с засовиванием данного текста в file.txt и отдавать уже файл
...
async def audio_file_handle(update: Update, context: CallbackContext):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        tmp_dir = Path(tmp_dir)
        audio_path = tmp_dir / f"{audio.file_id}.ogg"
        await audio.get_file().download(audio_path)

        # Transcribe the audio file
        transcribed_text = await openai_utils.transcribe_audio(audio_path)
...
def run_bot() -> None:
    ...
    dispatcher.add_handler(CommandHandler("transcribe_audio", audio_file_handle, filters=user_filter))
